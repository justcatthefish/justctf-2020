...
func CreateSandbox(name string, cpu int) error {
	timeoutStr := fmt.Sprintf("%d", int((config.Config.SandboxDuration + (time.Second*30)).Seconds()))
	cpuAssign := fmt.Sprintf("%d", cpu)
	cmd := exec.Command(config.Config.CreateSandboxSh, name, cpuAssign, timeoutStr)
...
func SandboxHandler(d *db) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// assign cpu
		if err := d.SetUserCpu(userData, cpuFree); err != nil {
			logger.WithError(err).Error("post SetUserCpu")
			w.WriteHeader(500)
			fmt.Fprintf(w, InternalError)
			return
		}
